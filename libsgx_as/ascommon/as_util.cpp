//     0xD6,0xFF,0x40,0xB6,0x06,0xE6,0x17,0x07,0x93,0xC2,0xFB,0x98,0xB3,0x14,0x58,0x7F,
//     0x9C,0xFD,0x25,0x73,0x62,0xDF,0xEA,0xB1,0x0B,0x3B,0xD2,0xD9,0x76,0x73,0xA1,0xA4,
//     0xBD,0x44,0xC4,0x53,0xAA,0xF4,0x7F,0xC1,0xF2,0xD3,0xD0,0xF3,0x84,0xF7,0x4A,0x06,
//     0xF8,0x9C,0x08,0x9F,0x0D,0xA6,0xCD,0xB7,0xFC,0xEE,0xE8,0xC9,0x82,0x1A,0x8E,0x54,
//     0xF2,0x5C,0x04,0x16,0xD1,0x8C,0x46,0x83,0x9A,0x5F,0x80,0x12,0xFB,0xDD,0x3D,0xC7,
//     0x4D,0x25,0x62,0x79,0xAD,0xC2,0xC0,0xD5,0x5A,0xFF,0x6F,0x06,0x22,0x42,0x5D,0x1B
// };
    ...
    IppsRSAPublicKeyState **cert_rsa_pub_key)
{
    ...
    do
    {
        ...
        sig_size--;
        // for (uint32_t i = 0; i < sig_size; i++) printf("%02x ", sig_begin[i]); 
        // printf("sig_size:%d\n",sig_size);
        ...
        // printf("Intel SGX Attestation Report Signing publc key (%d)\n", rsa_n_size);
        // for (uint32_t i = 0; i < rsa_n_size; i++) printf("%02x ", rsa_n[i]); 
        // printf("\n");
    ...
    uint32_t ias_crt_size)
{
    ...
    if (!p_ias_res || ias_res_size == 0 ||
        ...
        printf("is_ias_report_valid input not valid\n%x %x\n%x %x\n%x %x\n",p_ias_res, ias_res_size,p_ias_sig,ias_sig_size,p_ias_crt,ias_crt_size);
        return false;
    }

    uint8_t sig[256];
    if (256 != base64_decode(p_ias_sig, ias_sig_size, sig)) {
        printf("base64_decode invalid\n");
        return false;
    }
    ...
    do
    {
        ...
        uint8_t *p_res_body = p_ias_res + ias_res_size - 1;
        uint32_t res_body_size = 1;
        while (*p_res_body != '{' and p_res_body > p_ias_res) {
            p_res_body -= 1;
            res_body_size += 1;
        }
        ipp_status = ippsRSAVerify_PKCS1v15(p_res_body, res_body_size, sig, &is_valid, rsa_pub_key, ippHashAlg_SHA256, buffer);
        BREAK_ON_IPP_ERROR(ipp_status)
        ...
        if (!is_valid) {
            ...
            //     printf("\n");
            // for (uint32_t i = 0; i < res_body_size; i++) printf("%c", p_res_body[i]);
            //     printf("\n");            
            // is_valid = true;
        }
    } while(0);
    ...
    uint32_t ias_crt_size)
{
    ...
    {
        printf("is_ias_report_valid not valid!\n");
        return false;
    }
    ...
    if (SGX_SUCCESS != sgx_sha256_msg(reinterpret_cast<const uint8_t*>(p_msg), msg_size,
        ...
        printf("sgx_sha256_msg not valid!\n");
        return false;
    }
